...
/* The body speaking first. Everything else in this firmware answers a
 * question; this is the one message nobody asked for.
 *
 * `clock_confident` is false because it has to be: a Pico has no RTC and no
 * network time, so `ts` is uptime and the host is told to prefer the moment
 * it arrived. Saying so is the difference between a timestamp that is wrong
 * and a timestamp that is wrong *and trusted*. */
static void send_button_event(uint32_t hold_ms) {
    char id[27], ts[32];
    ulid(id, sizeof id);
    uptime_iso(ts, sizeof ts);
    printf("{\"jsonrpc\":\"2.0\",\"method\":\"notifications/body/event\","
           "\"params\":{\"id\":\"%s\",\"ts\":\"%s\",\"clock_confident\":false,"
           "\"type\":\"button\",\"name\":\"gpio%d\","
           "\"payload\":{\"hold_ms\":%lu}}}\n",
           id, ts, BUTTON_PIN, (unsigned long)hold_ms);
}
...
static void send_describe(long id) {
    printf("{\"jsonrpc\":\"2.0\",\"id\":%ld,\"result\":{"
           ...
           /* "button" is the only way this body can hint that it emits
            * events at all. The descriptor has no vocabulary for "these are
            * the events I can send" -- a host learns a button exists by
            * someone pressing it. caps are advisory by design (H12), so this
            * is a hint and not a contract. */
           "\"fw\":\"%s\",\"caps\":[\"led\",\"button\""
#if LED_DIMMABLE
           ",\"dimmable\""
#endif
#ifdef SERVO_PIN
...
int main(void) {
    ...
    for (;;) {
        /* Poll the button on the same cadence as the input. A press is
         * reported whether or not anybody is asking, which is the whole
         * point -- an event is not a reply. */
        uint32_t hold_ms;
        if (button_poll(&hold_ms)) send_button_event(hold_ms);
