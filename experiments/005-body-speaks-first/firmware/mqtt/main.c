...
static char t_presence[TOPIC_MAX], t_rpc[TOPIC_MAX], t_reply[TOPIC_MAX], t_event[TOPIC_MAX];
...
/* An event, published from the MAIN loop rather than an lwIP callback --
 * which is the opposite of every other publish in this firmware, and the
 * reason it takes the lock. Everything else here answers a request and is
 * already running inside lwIP; a button press answers nothing.
 *
 * clock_confident is false: a Pico W has no RTC, and although it has a
 * network it does not speak NTP. ts is uptime, and the host is told to
 * prefer arrival time. */
static void send_button_event(uint32_t hold_ms) {
    if (!mqtt_client_is_connected(client)) return;   /* nobody to tell */
    char id[27], ts[32];
    static char buf[256];
    ulid(id, sizeof id);
    ...
    mqtt_publish(client, t_event, buf, strlen(buf), 1, 0, NULL, NULL);
    cyw43_arch_lwip_end();
}
...
int main(void) {
    ...
    snprintf(t_event, sizeof t_event, "obp/body/%s/event", body_id);
    printf("body %s\n", body_id);
    ...
    while (true) {
        uint32_t hold_ms;
        if (button_poll(&hold_ms)) {
            printf("button held %lums\n", (unsigned long)hold_ms);
            send_button_event(hold_ms);
        }
        ...
        sleep_ms(5);   /* the button is polled here, so stay responsive */
    }
}
