...
class MqttConnection:
    ...
    def _on_message(self, client, userdata, msg) -> None:  # noqa: ANN001
        ...
        if kind == "event":
            # The body speaking first. This topic has been subscribed to
            # since experiment 003 and every message on it was discarded --
            # the return path existed on the wire and stopped at the host.
            try:
                obj = json.loads(msg.payload)
            except ValueError:
                return
            if self.on_event and obj.get("method") == "notifications/body/event":
                self.on_event(body_id, obj.get("params") or {})
