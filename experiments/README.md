...
| [002](002-agent-drives-body/) | An agent drives a body | Can an unmodified agent drive an OBP body with no code of ours inside it — and is MCP or a plain CLI the better join? | 🚧 CLI proven with a real agent; MCP-with-agent pending | **`userOnly` is about who is asking** · **a verb's description carries the decision, not its name** · advice written for humans (`newgrp`) silently breaks agents |
